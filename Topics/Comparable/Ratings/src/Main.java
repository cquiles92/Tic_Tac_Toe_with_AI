...
class Rating implements Comparable<Rating> {
    ...
    public int compareTo(Rating rating) {
        ...
        int netRatingA = this.getUpVotes() - this.getDownVotes();
        int netRatingB = rating.getUpVotes() - rating.getDownVotes();
        if (netRatingA > netRatingB) {
            return 1;
        } else if (netRatingA < netRatingB) {
            return -1;
        } else {
            return 0;
        }
